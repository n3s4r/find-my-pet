...
// Query to get all lost pets, ordered by the most recent ones first.
$all_lost_pets_query = "SELECT * FROM pets ORDER BY created_at DESC";
$all_lost_pets_result = $conn->query($all_lost_pets_query);

// *** NEW: Add more detailed error handling for the main query ***
if (!$all_lost_pets_result) {
    // Show a detailed error message and stop execution
    die("Error fetching lost pets: " . $conn->error);
}
...
<head>
    ...
    <title>Lost Pets Feed | Find My Pet</title>
...
<div class="container feed-container">
    <div class="text-center mb-5">
        <h2 style="font-weight: 700; color: #4353ff;">Lost Pets in Your Community</h2>
        <p class="text-muted lead">Browse the list of pets that have been reported lost by other users.</p>
    </div>

    <?php if ($all_lost_pets_result && $all_lost_pets_result->num_rows > 0): ?>
        <div class="row row-cols-1 row-cols-md-2 row-cols-lg-3 g-4">
            <?php while ($pet = $all_lost_pets_result->fetch_assoc()): ?>
                <div class="col">
                    <div class="card feed-card h-100">
                        ...
                        <div class="card-body">
                            ...
                            <ul class="list-unstyled mt-3">
                                <li><strong>Last Seen:</strong> <?= htmlspecialchars($pet['last_seen_location']) ?></li>
                                <li><strong>Date Lost:</strong> <?= date("F j, Y", strtotime($pet['last_seen_date'])) ?></li>
<!--                                <li><strong>Contact:</strong> <?= isset($pet['contact_info']) ? htmlspecialchars($pet['contact_info']) : 'N/A' ?></li>
                        -->
                            ...
                            <div class="comments-section">
                                ...
                                <?php
                                // Fix: Removed 'lpc.created_at' as it seems to be an invalid column name.
                                $comments_query = $conn->prepare("SELECT lpc.comment, u.full_name FROM lost_pet_comments lpc JOIN users u ON lpc.user_id = u.id WHERE lpc.pet_id = ?");
                                ...
                                <!-- Comment Form -->
                                <form action="add_lost_pet_comment.php" method="POST" class="mt-3">
    ...
    <?php else: ?>
        <div class="text-center py-5 px-3">
            <p class="lead text-muted">No lost pets have been reported yet. Keep an eye out!</p>
            <a href="add_pet.php" class="btn btn-primary mt-2">Report a Lost Pet</a>
        </div>
    <?php endif; ?>
</div>
